...
public class GenerateSentence
{
    ...
    {"ぢぃ", new string[3] {"dyi", "dilyi", "dixyi"}},
    ...
    {"っきゃ", new string[15] {"kkya", "kkilya", "kkixya", "ltukya", "ltukilya", "ltukixya", "xtukya", "xtukilya", "xtukixya", "ltsukya",
                            "ltsukilya", "ltsukixya", "xtsukya", "xtsukilya", "xtsukixya"}},
    ...
    {"っぎぃ", new string[25] {"ggyi", "ggili", "ggilyi", "ggixi", "ggixyi", "ltugyi", "xtugyi", "ltsugyi", "xtsugyi", "ltugili",
                                "xtugili", "ltsugili", "xtsugili", "ltugilyi", "xtugilyi", "ltsugilyi", "xtsugilyi", "ltugixi", "xtugixi", "ltsugixi",
                                "xtsugixi", "ltugixyi", "xtugixyi", "ltsugixyi", "xtsugixyi"}},
    ...
    /// <summary>
    /// ひらがな文をローマ字文字列に変換
    /// <param name="sentenceHiragana">ローマ字変換されるひらがな文字列</param>
    /// <returns>ローマ字文字列</returns>
    /// </summary>
    private static string ExchangeRoman(string sentenceHiragana)
    {
        StringBuilder roman = new StringBuilder(); // ローマ字を結合するためのStringBuilder

        int startIndex = 0;
        int endIndex = 3; // 3文字のひらがなに対する優先度を持つため、最初は3文字を試す

        while (startIndex < sentenceHiragana.Length)
        {
            bool foundMatch = false;

            // endIndexから1文字ずつひらがなを減らして試す
            for (int i = endIndex; i >= 1; i--)
            {
                if (startIndex + i <= sentenceHiragana.Length)
                {
                    string hiraganaStr = sentenceHiragana.Substring(startIndex, i);

                    if (romanTypeMap.ContainsKey(hiraganaStr))
                    {
                        // 最初のローマ字を選択
                        string[] romanOptions = romanTypeMap[hiraganaStr];
                        roman.Append(romanOptions[0]);
            ...
            {
                // 辞書に該当しない場合、そのまま結合
                roman.Append(sentenceHiragana[startIndex]);
                startIndex++;
            }
        }
    ...
    /// <summary>
    /// ひらがな文をパースして、判定を作成。ついでにローマ字変換追加
    /// <param name="sentenceHiragana">ひらがな文字列</param>
    /// <returns>判定器、ローマ字</returns>
    /// </summary>
    private static (List<List<string>> judgeReturn, StringBuilder romanReturn) ConstructTypeSentence(string sentenceHiragana)
    {
        int i = 0;
        string uni = "";
        string bi = "";
        ...
        while (i < sentenceHiragana.Length)
        {
            ...
            if (romanTypeMap.ContainsKey(tri))
            {
                ...
                i += 3;
                // 最初のローマ字を選択
                string[] romanOptions = romanTypeMap[tri];
                roman.Append(romanOptions[0]);
            }
            // 2文字でのマッチング
            else if (romanTypeMap.ContainsKey(bi))
            {
                ...
                i += 2;
                // 最初のローマ字を選択
                string[] romanOptions = romanTypeMap[bi];
                roman.Append(romanOptions[0]);
            }
            else
            {
                ...
                // 最初のローマ字を選択
                string[] romanOptions = romanTypeMap[uni];
                roman.Append(romanOptions[0]);
    ...
    /// <summary>
    /// タイピング表示に必要なひらがな文からローマ字と、判定器を生成
    /// <returns>(原文, ローマ字文, タイピング判定器)</returns>
    /// </summary>
    public static (bool isGenerateSuccess, string typeSentence, List<List<string>> typeJudge) Generate(string hiragana)
    {
        bool isOK = false;
        StringBuilder romanSentence;
        ...
        try
        {
//            typeSentenceStr = ExchangeRoman(hiragana);
//            retTypeJudge = ConstructTypeSentence(hiragana);
            (retTypeJudge, romanSentence) = ConstructTypeSentence(hiragana);
            typeSentenceStr = romanSentence.ToString();
            isOK = true;
        }
        catch
        {
        ...
        return (isOK, typeSentenceStr, retTypeJudge);
    }
